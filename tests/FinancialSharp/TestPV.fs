...
type TestPV() =
    ...
    member _.``pv with paymentDuePeriod = PaymentDuePeriod.Begin``(rate:double, nper:double, pv:double, fv:double, expectedResult:double) = 
        let result = Financial.PV(rate, nper, pv, fv, PaymentDuePeriod.Begin)
    ...
    member _.``pv with paymentDuePeriod = PaymentDuePeriod.End``(rate:double, nper:double, pv:double, fv:double, expectedResult:double) = 
        let result = Financial.PV(rate, nper, pv, fv, PaymentDuePeriod.End)
        Assert.AreEqual(expectedResult, result)
    
    [<Test>]
    member _.``pv with rate 0.0``() = 
        let result = Financial.PV(0.0, 20.0, 12000.0, 0.0)
        Assert.AreEqual(-240000.0, result)
    
    [<Test>]
    member _.``pv with rate not 0.0``() = 
        let result = Financial.PV(0.0, 20.0, 12000.0, 1000.0)
        Assert.AreEqual(-241000.0, result)